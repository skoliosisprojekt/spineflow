      ...
      <View style={styles.chartCard}>
        <View style={styles.chartHeader}>
          ...
          <Pressable onPress={showChartInfo} hitSlop={8} accessibilityRole="button" accessibilityLabel="Chart info" style={{ marginLeft: 'auto' }}>
            <MaterialIcons name="info-outline" size={16} color={C.text4} />
          </Pressable>
        </View>
...
function makeStyles(C: ThemeColors) {
  return StyleSheet.create({
    ...
    date: { fontSize: 13, color: C.text3, marginTop: 2 },
    settingsButton: { width: 48, height: 48, borderRadius: 24, backgroundColor: C.card, justifyContent: 'center', alignItems: 'center', marginRight: 40 },

    statsRow: { flexDirection: 'row', gap: 10, marginBottom: 14 },
    statCard: { flex: 1, backgroundColor: C.card, borderRadius: 14, padding: 14, alignItems: 'center', gap: 4 },
    statValue: { fontSize: 22, fontWeight: '800' },
    statLabel: { fontSize: 10, fontWeight: '600', color: C.text3, textTransform: 'uppercase' },

    activeWorkoutBanner: {
      flexDirection: 'row', alignItems: 'center', backgroundColor: C.accent,
      borderRadius: 14, padding: 16, marginBottom: 14, gap: 12,
    },
    activeDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#FFFFFF' },
    activeTitle: { fontSize: 15, fontWeight: '700', color: '#FFFFFF' },
    activeSubtitle: { fontSize: 12, color: 'rgba(255,255,255,0.8)' },

    card: { backgroundColor: C.card, borderRadius: 16, padding: 20, marginBottom: 14 },
    safetyCard: { borderLeftWidth: 4, borderLeftColor: C.accent },
    cardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 8, gap: 8 },
    cardTitle: { fontSize: 16, fontWeight: '700', color: C.text },
    cardDescription: { fontSize: 13, color: C.text2, lineHeight: 19 },

    profileRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 8 },
    profileLabel: { fontSize: 13, color: C.text3 },
    badge: { backgroundColor: C.accentLight, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 3 },
    badgeText: { fontSize: 12, fontWeight: '600', color: C.accentDark },
    separator: { height: 1, backgroundColor: C.bg },

    chartCard: { backgroundColor: C.card, borderRadius: 16, padding: 16, paddingBottom: 8, marginBottom: 14 },
    chartHeader: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 8 },
    chartTitle: { fontSize: 14, fontWeight: '700', color: C.text },
    chartSubtitle: { fontSize: 11, color: C.text3, marginLeft: 'auto' },
  });
}
